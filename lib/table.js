define(function (require, exports, module) {
  ...
  function getReferences() {
    ...
    var LOOKAHEAD = "(?=(" + AUTHOR + ")|\\n|\\r|$|(" + JOURNAL + DATE + "))";
    var DETAILS = "(.*?)";
    var queryInfo = {
//      query: "((\\S+,\\s+(?:[A-Z]\\.\\s*)+,?(?:\\s\\&|\\sand)?\\s+){1,10})\\(?(\\d\\d\\d\\d[a-z]?)[\\)\\.](.*?)(?=(\\S+,\\s+(?:[A-Z]\\.))|\\n|\\r|$|((\\s\\S+){1,5}\\.\\s\\(?(\\d\\d\\d\\d[a-z]?)[\\)\\.]))",
//      query: "((\\S+,\\s+(?:[A-Z]\\.\\s*)+,?(?:\\s\\&|\\sand)?\\s+){1,10})" + DATE + DETAILS + "(?=(\\S+,\\s+(?:[A-Z]\\.))|\\n|\\r|$|([^\\.]+\\.\\s\\(?(\\d\\d\\d\\d[a-z]?)[\\)\\.]))",
//      query: "((" + AUTHOR + "+,?(?:\\s\\&|\\sand)?\\s+){1,10})" + DATE + DETAILS + "(?=(\\S+,\\s+(?:[A-Z]\\.))|\\n|\\r|$|([^\\.]+\\.\\s\\(?(\\d\\d\\d\\d[a-z]?)[\\)\\.]))",
      query: AUTHOR_LIST + DATE + DETAILS + LOOKAHEAD,
//      query: AUTHOR_LIST + DATE + DETAILS + LOOKAHEAD;
      caseSensitive: true,
      isRegexp: true
    };
